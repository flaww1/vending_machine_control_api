...
async function getAllMaintenanceRequests(
    ...
    switch (sort) {
        ...
        case "priority_desc":
            ...
            break;
        case "claimedBy_asc":
            sortingMethod.claimedById = "asc";
            break;
        case "claimedBy_desc":
            sortingMethod.claimedById = "desc";
            break;
        case "machine_asc":
    ...
    if (claimedById) {
        filterSelection.claim = { equals: claimedById };
    }
    ...
    requests = await prisma.maintenanceRequest.findMany({
        select: {
            ...
            claimedById: true,
            observations: true,
            include: { Admin: { include: { User : { select: { first_name: true } } }
                , Company: { select: { name: true } }
                , Provider: { include: { User : { select: { first_name: true } } } }
        },
...
async function getAllRestockRequests(
    limit = 50, page = 1, sort, status, claimedById, priority
) {
    let filterSelection = {};
    let sortingMethod = {};
    ...
    switch (sort) {
        case "newest":
            sortingMethod.restockId = "desc";
            break;
        case "oldest":
            sortingMethod.restockId = "asc";
            break;
        case "type_asc":
            sortingMethod.type = "asc";
            break;
