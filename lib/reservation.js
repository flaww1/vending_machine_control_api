...
async function getAllReservations() {
    try {
        ...
        console.error('Error while retrieving reservations:', e.message);
...
async function updateReservation(reservationId, params) {
    try {
...
async function checkProductAvailabilityInMachine(machineId, productIds) {
    try {
        const machine = await prisma.machine.findUnique({
            ...
            include: {
                shelf: { // Updated field name to "shelf"
                    include: {
                        reservation: {
        ...
        // Check if the machine and its shelf exist
        if (!machine || !machine.Shelf) {
            throw new Error('Machine or shelf not found');
        }

        // Perform the necessary checks on the availability of the product in the machine

        return true; // Return availability status
    } catch (error) {
        console.error('Error while checking product availability:', error);
        throw new Error('Failed to check product availability in the machine');
    }
}
