...
async function getAllMachineModels() {
    try {
        return await prisma.MachineModel.findMany();
    } catch (e) {
        report(e);
        return null;
    }
...
async function getMachinesByProductId(productId) {
    try {
        ...
        product.Product_Shelf.forEach((productShelf) => {
            ...
            console.log(machineId, machine); // check the machineId and the machine
            ...
            if (!machine) {
                ...
                console.log(machinesMap); // check the machinesMap after a machine has been added
            }
            ...
            console.log(machine); // check the machine after a shelf has been added
        });
...
// only admin can delete products
    ...
    async function getReservationsByUserId (userId) {
        try {
            return reservation = await prisma.reservation.findMany({
                where: {
                    userId: userId
                }
